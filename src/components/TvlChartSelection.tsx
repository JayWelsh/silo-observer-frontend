  ...
  useEffect(() => {
    ...
    Promise.all([
        ...((tokenSymbol && deploymentID) ? [
          ...
          fetch(`${API_ENDPOINT}/tvl-totals/mv/${`whole-platform?networks=${selectedNetworkIDs.join(",")}&`}perPage=32000`).then(resp => resp.json()),
          fetch(`${API_ENDPOINT}/borrowed-totals/mv/${`whole-platform?networks=${selectedNetworkIDs.join(",")}&`}perPage=32000`).then(resp => resp.json()),
        ]),
    ]).then((data) => {
